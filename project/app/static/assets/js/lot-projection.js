...
function renderWorkspace(rows,content,title,subtitle){
 ...
 function refreshTables(){
  ...
  historyA.innerHTML=historyTable(ga,"#2563eb",pending);historyB.innerHTML=historyTable(gb,"#ef4444",pending);
 ...
 content.addEventListener("input",event=>{
  const input=event.target.closest("[data-rest-input]");if(!input)return;
  const key=input.dataset.lotId+":"+input.dataset.assetId;
  const saved=rows.find(row=>String(row.lot_id)===input.dataset.lotId&&String(row.media_asset_id)===input.dataset.assetId);
  if(saved&&number(input.value)===number(saved.rest_days))pending.delete(key);else pending.set(key,input.value);
 });
 document.getElementById("projectionSaveRest").addEventListener("click",async()=>{
  const button=document.getElementById("projectionSaveRest");
  if(!pending.size){message.textContent="No hay cambios para guardar.";message.classList.remove("hidden");return}
  for(const value of pending.values()){
   if(value.trim()===""||number(value)===null||number(value)<0){message.textContent="Escribe un número válido de días de descanso en todos los campos modificados.";message.classList.remove("hidden");return}
  }
  button.disabled=true;button.innerHTML='<i class="fas fa-spinner fa-spin mr-2"></i>Guardando...';
  let savedCount=0;
  try{
   for(const [key,rawValue] of Array.from(pending)){
    const [lotId,assetId]=key.split(":").map(Number),value=number(rawValue);
    ...
    pending.delete(key);savedCount++;
   }
   graph.innerHTML="";message.textContent=savedCount+" valor(es) guardado(s). Pulsa Graficar para actualizar.";message.classList.remove("hidden");
  }catch(error){message.textContent=savedCount+" valor(es) guardado(s). "+error.message+" Los cambios pendientes siguen disponibles.";message.classList.remove("hidden")}
  finally{refreshTables();button.disabled=false;button.innerHTML='<i class="fas fa-save mr-2"></i>Guardar días'}
 });
