...
@jwt_required()
@limiter.limit(
    f"{CoreConfig.RATE_LIMIT_PROFILE_GET} per minute", key_func=get_limit_key
)
def get_profile_v1():
...
@jwt_required()
@limiter.limit(
    f"{CoreConfig.RATE_LIMIT_PROFILE_UPDATE} per minute", key_func=get_limit_key
)
...
@jwt_required()
@limiter.limit(
    f"{CoreConfig.RATE_LIMIT_AVATAR_UPLOAD} per minute", key_func=get_limit_key
)
def upload_avatar():
    """Upload avatar image.
    ...
    try:
        ...
        current_app.logger.error(
            f"Failed to commit avatar update for user {user.id}: {str(e)}"
        )
    ...
    if old_avatar_path and old_avatar_path != new_avatar_path:
        ...
        if not deleted:
            current_app.logger.warning(
                f"Failed to delete old avatar file: {old_avatar_path}"
            )
    ...
    return (
        jsonify({"avatar_url": avatar_url, "message": "Avatar uploaded successfully"}),
        200,
    )
...
@jwt_required()
@limiter.limit(
    f"{CoreConfig.RATE_LIMIT_AVATAR_DELETE} per minute", key_func=get_limit_key
)
def delete_avatar():
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Failed to commit avatar removal for user {user.id}: {str(e)}"
        )
