...
def element(objective_id: int):
    ...
    thumb_url = (
        url_for("media.serve_file", key=thumb_variant.storage_key)
        if thumb_variant
        else asset_url
    )
    ...
    preview_defs = [
        ...
        (
            "nd_index_rgba",
            f"{asset.uuid}__nd_index_rgba.png",
            "Índice espectral (RGBA)",
        ),
    ]
    preproc_previews = []
...
def element_reprocess(objective_id: int):
    ...
    flash(
        "Se encoló el reprocesamiento del activo. Este proceso puede tardar varios minutos según el tamaño del archivo.",
        "info",
    )
        ...
        try:
            ...
            flash(
                (
                    ...
                    if created
                    else "El archivo ya existía y se reutilizó el registro."
                ),
                "success",
            )
            return render_template(
