...
def save_quick_preview(
    ...
    This function replicates the legacy Agrovista preview pipeline:
    1. Read RGB (handling 8/16 bit, NoData).
    2. Replace NaN with 0 (black).
    3. Clip to [0, 1].
    4. Scale to uint8 [0, 255].
    5. Resize (thumbnail) to max_size.
    6. Save as PNG.
    """
    # Use media's read_rgb which is robust for 8/16 bit and NoData
    rgb = read_rgb(src_path)

    # Handle NaN -> 0 (black) and clip
...
def preprocess_rgb_once(
    ...
    def ensure_rgb_preview() -> Optional[Path]:
        if png_path.exists():
            return png_path
        save_quick_preview(input_path, png_path, max_size=cfg.preview_max_dim)
        return png_path
    ...
    if npz_path.exists():
        try:
            ...
            try:
                ...
                pass
            vi_cache = cache_dir / f"{in_key}__vi_gr_ratio.png"
            try:
                vi_cache.unlink()
            except OSError:
                pass
            vi_heat_cache = cache_dir / f"{in_key}__vi_gr_heat.png"
            try:
                vi_heat_cache.unlink()
            except OSError:
                pass
            heatmap_cache = cache_dir / f"{in_key}__vi_heatmap.png"
            try:
                heatmap_cache.unlink()
            except OSError:
                pass
    ...
    np.savez_compressed(npz_path, rgb=rgb_lin.astype(np.float32))
